...
const HomePage = () => {
    ...
    const getVideo = (link, id) => {
        ...
        fetch(process.env.REACT_APP_API_VIDEO, {
            method: 'POST',
            headers: {
    ...
    const convertToMp3 = (starttime, endtime) => {
        ...
        fetch(process.env.REACT_APP_API_CONVERT, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            ...
            .catch(err => {
                setError('File does not exists anymore, try again by refreshing the page');
            })
            .finally(() => {
                setIsLoading(false);
            })
    ...
    return (
        <>
            {!showVideo
                ...
                : <ConvertWrapper videolink={videolink} goBackToStart={goBackToStart} convertToMp3={convertToMp3} />
            }
        </>
    )
}
